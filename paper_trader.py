...
class SimulatedExchange:
    def __init__(self, initial_balance=10000, fee_percent=0.001):
        ...
        self.trades = []
        logging.info(
            f"Simulated exchange initialized with ${initial_balance:,.2f} USD."
        )
...
def log_portfolio_update(exchange, current_prices):
    ...
    holdings_str = ", ".join(
        [
            f"{amount:.6f} {symbol}"
            for symbol, amount in exchange.portfolio.items()
            if amount > 0
        ]
    )
    if not holdings_str:
    ...
    try:
        while True:
            ...
            for ticker in tickers:
                ...
                # --- Signal Generation ---
                data["rsi"] = calculate_rsi_manual(
                    data["Close"], period=best_params["rsi_period"]
                )
                ...
                if positions[ticker] is None:
                    ...
                    rsi_confirmation = current_rsi >= (
                        50 + best_params["rsi_confirmation_threshold"]
                    )

                    if rsi_crossed_above and rsi_confirmation:
                        logging.info(
                            f"Potential BUY signal for {ticker} at ${current_price:,.2f}"
                        )
                        position_size_usd = exchange.balance_usd * 0.20
                        if exchange.execute_buy(
                            ticker, position_size_usd, current_price
                        ):
                            positions[ticker] = {
                                "type": "long",
                ...
                # --- Exit Logic ---
                    ...
                    if rsi_crossed_below and rsi_confirmation:
                        logging.info(
                            f"Potential SELL signal for {ticker} at ${current_price:,.2f}"
                        )
                        amount_to_sell = exchange.portfolio.get(ticker, 0)
                        if amount_to_sell > 0:
                            if exchange.execute_sell(
                                ticker, amount_to_sell, current_price
                            ):
